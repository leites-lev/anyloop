 ...
 *
 * Mirrors fsp.c: Smith reconstruction (1597), boxcar prefilter (1622),
 * dual NLMS predictors blended at delay_frac (1667/1697), command (1802).
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(int argc, char **argv)
{
	...
	for (size_t i = 0; i < n; i++) {
		...
		if (i >= n0) {
			...
			if (t > hold) {
				double s1 = muc * (bl[i] - p1) / en1;
				double s2 = muc * (bl[i] - p2) / en2;
				double lk = 1.0 - leak;
				for (int k = 0; k < P; k++) {
					w[k]  = w[k]  * lk + s1 * bl[i0 - k];
					wn[k] = wn[k] * lk + s2 * bl[i0 - 1 - k];
				}
			}
			double c1 = 0, c2 = 0;
			for (int k = 0; k < P; k++) {
...
/* BUILD:  gcc -O3 -march=native -ffast-math -o fsp_replay contrib/fsp_replay.c -lm
 *
 * USAGE:  fsp_replay K_true K_mod d_int d_frac mu mu_init mu_tau leak lp order [noise_sigma]
